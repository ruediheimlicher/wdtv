...
- (void)browserCellSelected:(id)sender // double Click
{
   ...
   filmURL = [[self.tvbrowser itemAtRow: index inColumn:l-1]URL];
    NSLog(@"filmLink: %@  length: %lu index: %lu l: %lu",filmLink, (unsigned long)[filmLink length],index, l);
   if ([filmLink length])
   {
...
-(NSString *) randomStringWithLength: (long) len
{
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
   ...
   self.VolumesView.textColor = [NSColor greenColor];
   self.VolumesView.editable = NO;
   self.VolumesView.selectable = NO;
   self.VolumesView.font = [NSFont boldSystemFontOfSize:14];
   self.VolumesView.font = [NSFont fontWithName:@"Helvetica" size:14];

   NSFileManager* fm = [NSFileManager defaultManager];
   FilmVolumesArray = [[NSMutableArray alloc]initWithCapacity:0]; // Order mit Pfaden zu Volumes mit Filmordnern
   ...
   for (NSURL * mountPath in mountPaths)
   {
      ...
      // Volumes suchen
       BOOL isDir=0;
      if ([[mountPath path] containsString:@"Volumes"] && (![sperrordnernamenarray containsObject:[[mountPath path]lastPathComponent]]))
      {
         //NSLog(@"Volume: mountPath pfad: %@ ",[mountPath path]);
         ...
         */
         ...
         if ([tempVolumeArray count])
         {
            ...
            for (NSURL* tempURL in tempVolumeArray)
            {
               NSString* pfad = [[tempURL path] lastPathComponent ];
               ...
               {
                  NSURL* filmarchivURL = tempURL;
                  //NSLog(@"filmarchivURL: %@",[filmarchivURL path]);
                  
                  NSArray *tempOrdnerArray = [[NSFileManager defaultManager] contentsOfDirectoryAtURL:tempURL
                                                                           includingPropertiesForKeys:[NSArray arrayWithObject:NSURLNameKey]
                                                                                              options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                                                error:nil];
                  //NSLog(@"tempOrdnerArray: %@ ",tempOrdnerArray);
                  for (NSURL * tempURL in tempOrdnerArray )
                  {
                     [FilmOrdnerArray addObject:tempURL];
                  }
...
*/
   ...
   self.Host_Name = [NSString stringWithCString:host_name encoding:NSUTF8StringEncoding];
   ...
   self.FilmSpeicherArray = [[NSMutableArray alloc]initWithCapacity:0]; // Files auf Film_HD an mini
   ...
   Doppel_Array = [[NSMutableArray alloc]initWithCapacity:0]; // Sichtbare Volumes
  Denied_Array = [[NSMutableArray alloc]initWithCapacity:0]; // Sichtbare Volumes
   
   
   [self.tvbrowser setTarget:self];
   [self.tvbrowser setColumnResizingType:NSBrowserUserColumnResizing];
   [self.tvbrowser sizeToFit];
//   [self.tvbrowser  setWidth:[self.tvbrowser columnWidthForColumnContentWidth:250] ofColumn:0];

   [self.tvbrowser  setWidth:[self.tvbrowser columnWidthForColumnContentWidth:250] ofColumn:1];
 ...
 //  [[self.tvbrowser matrixInColumn:1] sizeToCells];
  ...
  // [self.volumepop addItemWithTitle:@"Volume wählen"];
   if ([Volumes_Array count])
   {
   [self.volumepop addItemsWithTitles:[Volumes_Array valueForKey:@"volume"]];
   [self.volumepop selectItemAtIndex:0];
   [self volumepopwahl];
   }
...
// Volumes nach filmlisten absuchen
- (void)RefreshFilmlisten
{
   ...
   NSLog(@"RefreshFilmlisten FilmSpeicherArray count: %lu",(unsigned long)self.FilmSpeicherArray.count);
   [[self FilmSpeicherArray]removeAllObjects];
   
   // Filme der gemounteten Volumes auflisten, geordnet nach Genre
   if (FilmOrdnerArray.count) // Pfade der Genre-Ordner auf Volumes
   {
      //NSLog(@"RefreshFilmlisten FilmOrdnerArray: %@",FilmOrdnerArray);
      
      //NSLog(@"FilmOrdnerArray da");
      
      for (NSURL* tempURL in FilmOrdnerArray)
      {
         NSLog(@"tempURL: %@",tempURL);
         
         NSString* tempOrdner = [[tempURL path]lastPathComponent];
         //NSString* tempVolumeA = [[tempURL path]stringByDeletingLastPathComponent];
         //NSLog(@"tempVolumeA: %@",tempVolumeA);
         //NSString* tempVolumeB = [[[tempURL path]stringByDeletingLastPathComponent] stringByAppendingPathComponent:tempOrdner];
         //NSLog(@"tempVolumeB: %@",tempVolumeB);
         
         //        NSArray* tempFilmArray =[self FilmarrayVonURL:[tempURL URLByDeletingLastPathComponent]];
         //        NSArray* tempFilmArray =[self FilmarrayVonURL:[[tempURL URLByDeletingLastPathComponent] URLByAppendingPathComponent:tempOrdner]];
         //NSArray* tempFilmArray =[self FilmarrayVonGenreURL:[[tempURL URLByDeletingLastPathComponent] URLByAppendingPathComponent:tempOrdner]];
         
         // Filme des Genre-Ordners auflisten
         NSArray* tempFilmArray =[self FilmarrayVonGenreURL:tempURL];
         
         // Liste zu FilmSpeicherArray addieren
         if ([tempFilmArray count])
         {
               // NSLog(@"tempFilmArray: %@",tempFilmArray);
            
            [self.FilmSpeicherArray addObject: [NSDictionary dictionaryWithObject:tempFilmArray forKey:[tempURL path]]];
         }
         
      } // for in FilmOrdnerArray
   ...
   // Pfad-Liste aller Filme auf mounted Volumes
   NSString* titelString = [self titelListeAusFilmListe:self.FilmSpeicherArray];
   //NSLog(@"titelString: \n%@",titelString);
   
   if ([self.FilmSpeicherArray count])
   {
      // Film_Liste in Documents/WDTVDaten lesen: old_titelString
      NSError* err;
      NSString* filmlistePfad=[NSHomeDirectory() stringByAppendingFormat:@"%@%@%@",@"/Documents",@"/WDTVDaten",@"/Film_Liste.txt"];
      NSString* old_titelString = [NSString string];
      NSStringEncoding enc=NSUTF8StringEncoding;
      
      if ([[NSFileManager defaultManager]fileExistsAtPath:filmlistePfad])
      {
         NSURL* oldfileURL = [NSURL fileURLWithPath:filmlistePfad];
         NSError* err = nil;
         
         old_titelString = [NSString stringWithContentsOfURL:oldfileURL encoding:enc error:&err];
      }
      
      //NSLog(@"old_titelString: %@ encoding: %lu",old_titelString ,(unsigned long)enc);
      
      // Filmtitel als Array
      //NSMutableArray* ListeTitelArray = (NSMutableArray*)[old_titelString componentsSeparatedByString:@"\n"];
      ListeTitelArray = [[NSMutableArray alloc]initWithArray:[old_titelString componentsSeparatedByString:@"\n"]];
      
      //old_titelDicArray  Dic mit Pfad (pfad) und Titel (kurztitel) von jedem Film 
      NSMutableArray* old_titelDicArray = [[NSMutableArray alloc]initWithCapacity:0];
      for (NSString* tempZeile in ListeTitelArray)
      {
         if (tempZeile.length)
         {
            NSArray* tempTitelArray = [[[tempZeile componentsSeparatedByString:@"/"]lastObject]componentsSeparatedByString:@" "];
            ...
            {
               // Datumcode entfernen
               tempTitelArray = [tempTitelArray subarrayWithRange:NSMakeRange(1,tempTitelArray.count-2)];
               //NSString* tempTitel = [tempTitelArray componentsJoinedByString:@" "];
               NSString* shortTitel = [[[tempZeile componentsSeparatedByString:@"/"]lastObject] substringFromIndex:6];
               //NSLog(@"tempZeile: %@ shortTitel: %@",tempZeile,shortTitel);
               NSDictionary* tempTitelDic = [NSDictionary dictionaryWithObjectsAndKeys:shortTitel,@"kurztitel",tempZeile,@"pfad", nil];
               [old_titelDicArray addObject:tempTitelDic];
      ...
      NSLog(@"ListeTitelArray count: %ld",ListeTitelArray.count);
      NSLog(@"ListeTitelArray: %@",old_titelDicArray);
      NSArray* shortTitelArray =[old_titelDicArray valueForKey:@"kurztitel"];
      //NSLog(@"shortTitelArray: %@",shortTitelArray);
      NSSet *oldTitelSet = [NSSet setWithArray:shortTitelArray];
      
      NSMutableArray* new_titelStringArray = (NSMutableArray*)[titelString componentsSeparatedByString:@"\n"];
      NSMutableArray* new_titelDicArray = [[NSMutableArray alloc]initWithCapacity:0];
      NSMutableArray* new_titelArray = [[NSMutableArray alloc]initWithCapacity:0];
      for (NSString* tempZeile in new_titelStringArray)
      {
         if (tempZeile.length)
         {
            NSArray* tempTitelArray = [[[tempZeile componentsSeparatedByString:@"/"]lastObject]componentsSeparatedByString:@" "];
            ...
            else
            {
               ...
               //NSString* tempTitel = [tempTitelArray componentsJoinedByString:@" "];
               NSString* shortTitel = [[[tempZeile componentsSeparatedByString:@"/"]lastObject] substringFromIndex:6];
               //NSLog(@"new_titelArray tempZeile: %@ shortTitel: %@",tempZeile,shortTitel);
               if (!([oldTitelSet member:shortTitel]))
               {
                  NSDictionary* tempTitelDic = [NSDictionary dictionaryWithObjectsAndKeys:shortTitel,@"kurztitel",tempZeile,@"pfad", nil];
                  [new_titelDicArray addObject:tempTitelDic];
                  [new_titelArray addObject:tempZeile];
                  [ListeTitelArray addObject:tempZeile];
               }
            }
         ...
         {
            NSLog(@"new_titelArray Zeile leer");
         }
         
      }
      
      
      NSLog(@"new_titelArray count: %ld",new_titelArray.count);
      NSLog(@"ListeTitelArray count: %ld",ListeTitelArray.count);
      
      // http://stackoverflow.com/questions/1649198/objective-c-how-to-compare-arrays-and-extract-the-difference
      
      NSSet *oldSet = [NSSet setWithArray:[old_titelDicArray valueForKey:@"kurztitel"]];
      
      NSMutableSet *newSet = [NSMutableSet setWithCapacity:[new_titelArray count]];
      NSMutableSet *diffSet = [NSMutableSet setWithCapacity:[new_titelArray count]];
      [newSet addObjectsFromArray:new_titelArray];
      
      [newSet unionSet:oldSet]; // result is in `newSet`
      
      NSArray* unionArray = newSet.allObjects;
      //NSLog(@"unionArray count: %ld",unionArray.count);
      
      [diffSet addObjectsFromArray:new_titelArray];
      [diffSet minusSet:oldSet]; // result is in `diffSet`
      NSArray* diffArray = diffSet.allObjects;
      //NSLog(@"diffArray count: %ld diffArray: %@",diffArray.count,diffArray);
      int index=0;
      while ([[ListeTitelArray firstObject] length] ==0)
      {
         ListeTitelArray = (NSMutableArray*)[ListeTitelArray subarrayWithRange:NSMakeRange(1, ListeTitelArray.count-1)];
         index++;
         NSLog(@"ListeTitelArray count nach %d runs: %ld",index,ListeTitelArray.count);
      }
      
      NSString* newTitelString = [ListeTitelArray componentsJoinedByString:@"\n"];
      //  NSString* newTitelString = [new_titelArray componentsJoinedByString:@"\n"];
      newTitelString = [newTitelString stringByTrimmingCharactersInSet:
                        [NSCharacterSet whitespaceCharacterSet]];
      
      //BOOL suc = [self.FilmSpeicherArray writeToFile: filmspeicherlistePfad atomically:YES];
      BOOL suc = [newTitelString writeToFile: filmlistePfad atomically:YES encoding:NSUTF8StringEncoding error:&err];
      NSLog(@"write newTitelString suc: %d error: *%@*",suc,err);
      
      
      
      //NSLog(@"self.FilmSpeicherArray count: %d",[self.FilmSpeicherArray count]);
      NSString* filmspeicherlistePfad=[NSHomeDirectory() stringByAppendingFormat:@"%@%@%@",@"/Documents",@"/WDTVDaten",@"/FilmSpeicher_Liste.txt"];
      //NSString* titelliste = [self titelListeAusArray:self.FilmSpeicherArray];
      
   }
   else
   {
      NSLog(@"RefreshFilmlisten: FilmSpeicherArray ist leer");
   }
   ...
   self.Mag_Pfad = [NSString stringWithFormat:@"/Volumes/Mag/Filmarchiv"];
...
- (NSString*)titelListeAusArray:(NSArray*)derFilmArray
{
   ...
   for (int i=0;i<derFilmArray.count;i++)
   {
      ...
      switch (tempElementeArray.count)
      {
         ...
         case 4:
         {
            NSLog(@" titelListeAusArray 4 Elemente : %@",tempElementeArray);
...
- (NSString*)titelListeAusFilmListe:(NSArray*)derFilmArray
{
   //NSLog(@"  ++  start titelListe");
   NSMutableArray* ListeArray = [[NSMutableArray alloc]initWithCapacity:0];
   ...
   for (int i=0;i < derFilmArray.count;i++)
   {
      ...
      NSDictionary* tempFilmDic = [derFilmArray objectAtIndex:i];
      
      NSArray* temprawTitelArray = tempFilmDic.allValues;
      NSArray* tempTitelArray = [temprawTitelArray objectAtIndex:0];

      // Titel filtern
      
      for (int k=0;k<tempTitelArray.count;k++)
      {
         
         NSString* tempFilm = [tempTitelArray objectAtIndex:k]; // WD_TV_A		Volumes	WD_TV_A	Tatort	Mag	Tatort 130323 Summ, Summ, Summ.mpg
         //NSString* tempFilmTitel = [[[filmArray objectAtIndex:i] stringByReplacingOccurrencesOfString:self.WDTV_Pfad withString:@""]substringFromIndex:1];// erstes tab weg
         
         //NSLog(@"tempFilmTitel: %@",tempFilm );
         NSArray* tempElementeArray = [tempFilm componentsSeparatedByString:@"/"]; // Am anfang steht ein /
         
         if ([[tempElementeArray objectAtIndex:1] isEqualToString:@"Volumes"])
         {
            tempElementeArray = [tempElementeArray subarrayWithRange:NSMakeRange(1, [tempElementeArray count]-1)];
         }
         //NSLog(@"tempFilmTitel: %@ anz: %d",tempFilm,[tempElementeArray count] );
         
         NSString* kontrolltitel = [[tempElementeArray lastObject]stringByDeletingPathExtension];
         ...
         switch (tempElementeArray.count)
         {
          ...
          //     NSLog(@"6 El : %@",[tempElementeArray lastObject]);
               NSMutableDictionary* tempZeilenDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                     ...
                                                     [tempElementeArray objectAtIndex:4],@"sub",
                                                     tempZeilenString,@"titelstring",
                                                     [tempElementeArray objectAtIndex:5],@"titel",
                                                     tempFilm,@"film",nil];
               
               NSString* titelteil = [[tempElementeArray objectAtIndex:5]stringByDeletingPathExtension];
               [TitelArray addObject:titelteil];
               NSString* titelnummer =0;
               //NSLog(@"Titel raw: %@",[[tempElementeArray objectAtIndex:5]stringByDeletingPathExtension]);
            ...
            case 5:// ohne Jahrzahl
            {
               ...
               //NSLog(@"tempElementeArray: %@ tempZeilenString: %@", tempElementeArray, tempZeilenString);
         //      NSLog(@"5 El : %@",[tempElementeArray lastObject]);
               NSString* titelteil = [[tempElementeArray objectAtIndex:4]stringByDeletingPathExtension];
               [TitelArray addObject:titelteil];
               ...
               NSMutableDictionary* tempZeilenDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                     ...
                                                     [tempElementeArray objectAtIndex:3],@"sub",
                                                     tempZeilenString,@"titelstring",
                                                     [tempElementeArray objectAtIndex:4],@"titel",
                                                     tempFilm,@"film",nil];
               [ListeArray addObject:tempZeilenDic];
               
            }break;
            case 4:// ohne Jahrzahl
            {
               
               NSString* tempZeilenString = [[tempElementeArray subarrayWithRange:NSMakeRange(1, 3)]componentsJoinedByString:@"\t"];
               //NSLog(@"tempElementeArray: %@ tempZeilenString: %@", tempElementeArray, tempZeilenString);
         //      NSLog(@"4 El : %@",[tempElementeArray lastObject]);
               NSString* titelteil = [[tempElementeArray lastObject]stringByDeletingPathExtension];
               [TitelArray addObject:titelteil];
               ...
               NSMutableDictionary* tempZeilenDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                     ...
                                                     [tempElementeArray objectAtIndex:3],@"sub",
                                                     tempZeilenString,@"titelstring",
                                                     [tempElementeArray objectAtIndex:3],@"titel",
                                                     tempFilm,@"film",nil];
               [ListeArray addObject:tempZeilenDic];
            ...
            default:
            {
               NSLog(@"titelListeAusFilmListe falscher Titel: %@",tempFilm);
            }break;
         }// switch
      } // for k
   }// for i
  ...
  //NSLog(@"TitelArray count: %d: \n%@",[TitelArray count],TitelArray );
   ...
   for (int i=0;i<ListeArray.count;i++)
   {
      ...
      NSArray* temparray =[[[ListeArray objectAtIndex:i]objectForKey:@"titelstring"]componentsSeparatedByString:@" "];
      if ([temparray count]>4)
      {
      temparray = [temparray subarrayWithRange:NSMakeRange(1, [temparray count]-1)];
      }
      ...
      if ([Liste containsString:temptitelstring])
      {
         ...
         NSString* doppelpfad = [[[[@"/Volumes"stringByAppendingPathComponent:[[ListeArray objectAtIndex:i]objectForKey:@"ort"]]
                                  stringByAppendingPathComponent:[[ListeArray objectAtIndex:i]objectForKey:@"art"]]
                                 stringByAppendingPathComponent:[[ListeArray objectAtIndex:i]objectForKey:@"sub"]]
                                 stringByAppendingPathComponent:[[ListeArray objectAtIndex:i]objectForKey:@"titel"]]
                                 ;
         NSLog(@"doppelpfad: %@",doppelpfad);
         doppelpfad =[[ListeArray objectAtIndex:i]objectForKey:@"film"];
         NSLog(@"doppelpfad neu: %@",doppelpfad);

         if ([[NSFileManager defaultManager]fileExistsAtPath:doppelpfad])
         {
            NSLog(@"file an pfad %@ ist da",doppelpfad);
            NSString *question = NSLocalizedString(@"Double Film", @"Doppelter Film");
            NSString *info = doppelpfad ;
            NSString *deleteButton = NSLocalizedString(@"Delete", @"Loeschen");

            NSString *continueButton = NSLocalizedString(@"Ignore", @"Ignorieren");
            ...
            if (answer == 1000) // 1000, delete
            {
               ...
               {
                  NSError* err;
                  BOOL suc = [[NSFileManager defaultManager]removeItemAtPath:doppelpfad error:&err];
                  NSLog(@"file an pfad %@ ist weg:suc: %d err: %@",doppelpfad,suc,err);
                  if (suc == 0)
                  {
                     [Denied_Array addObject:doppelpfad];
                  }
         ...
         [Doppel_Array addObject:[[ListeArray objectAtIndex:i]objectForKey:@"film"]];
      }
//      Liste = [Liste stringByAppendingFormat:@"%@\t%@\n",[[ListeArray objectAtIndex:i]objectForKey:@"ort"],[[ListeArray objectAtIndex:i]objectForKey:@"titelstring"]];
      Liste = [Liste stringByAppendingFormat:@"%@\n",[[ListeArray objectAtIndex:i]objectForKey:@"film"]];
   }
   //NSLog(@"Liste: \n%@",Liste);
   if ([Doppel_Array count])
   {
      NSLog(@"titelListeAusFilmListe Doppel_Array: \n%@",Doppel_Array);
   }
   if ([Denied_Array count])
   {
   NSLog(@"titelListeAusFilmListe Denied_Array: \n%@",Denied_Array);
   }
   //NSLog(@"  ++  end titelListe");
...
- (NSArray*)FilmeAnPfad:(NSString*)pfad
{
   ...
   if ([Filemanager fileExistsAtPath:pfad])//ist
   {
      ...
      NSArray* tempFilmOrdnerArray=  [Filemanager contentsOfDirectoryAtURL:[NSURL fileURLWithPath:pfad]
                                              includingPropertiesForKeys:[NSArray arrayWithObject:NSURLNameKey]
                                                                 options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                   error:&err];
      NSLog(@"FilmeAnPfad tempFilmOrdnerArray: %@",tempFilmOrdnerArray);
      
      BOOL isDir;
      if ([tempFilmOrdnerArray count])
      {
         for (NSURL* unterorderurl0 in tempFilmOrdnerArray )
         {
...
- (NSArray*)FilmMag
{
 ...
 //       answer = [alert runModal];
      ...
      self.errorfeld.string = [[self.errorfeld string]stringByAppendingFormat:@"%@\n",@"Filmarchiv auf TC_Basis nicht da" ];

      NSLog(@"Filmarchiv auf TC_Basis nicht da");
   }
...
- (NSArray*)FilmarrayVonGenreURL:(NSURL*)Genre_URL
{
   {
      ...
      {
         NSArray* pfadcomponentenarray =[[Genre_URL path]componentsSeparatedByString:@"/"];// Pfadkomponenten
         
         // Genre_URL: /Volumes/HD_XY/Genre_xy oder /Volumes/HD_UV/Filmarchiv/Genre_xy
         NSString* filmDir = [Genre_URL path];
         NSString* filmVolume = [[[Genre_URL path]stringByDeletingLastPathComponent ]lastPathComponent]; //
         if ([filmDir containsString:@"Filmarchiv"])
         {
            // Volume ist eine Ebene hoeher, Filmarchiv abschneiden
            filmVolume = [[[[Genre_URL path]stringByDeletingLastPathComponent ]stringByDeletingLastPathComponent ]lastPathComponent];
         }
         else
         {
         
           // filmVolume = [[[Genre_URL path]stringByDeletingLastPathComponent ]lastPathComponent]; //
         }
    //     NSLog(@"Genre_URL: %@ filmVolume: %@",Genre_URL,filmVolume);
         [Volumes_Array addObject:[NSDictionary dictionaryWithObjectsAndKeys:[Genre_URL path] , @"path",filmVolume,@"volume", nil]];
         NSString* tempVolumesString = [self.VolumesView string];
         
         if ([tempVolumesString length])
         {
            if (![tempVolumesString containsString:filmVolume])
            {
               NSString* volumetext =[NSString stringWithFormat:@"%@\n%@",tempVolumesString,filmVolume];
               self.VolumesView.string = volumetext;
               self.errorfeld.string = [[self.errorfeld string]stringByAppendingFormat:@"%@ da\n",filmVolume ];
            }
         }
         else
         {
            self.VolumesView.string = filmVolume;
         ...
         NSArray* HD_OrdnerArray=  [Filemanager contentsOfDirectoryAtURL:Genre_URL
                                              includingPropertiesForKeys:[NSArray arrayWithObject:NSURLNameKey]
                                                                 options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                   error:&err];
         ...
         {
            for (NSURL* unterorderurl0 in HD_OrdnerArray )
            {
               //NSLog(@"Niveau 1 unterorderfad: %@",unterorderurl0);
               if([Filemanager fileExistsAtPath:[unterorderurl0 path] isDirectory:&isDir])
               {
                  // AAA
                  if (isDir) // Unterordner da (mag, intern)
                  {
                     NSArray* OrdnerArray1 =  [Filemanager contentsOfDirectoryAtURL:unterorderurl0
                                                         includingPropertiesForKeys:[NSArray arrayWithObject:NSURLNameKey]
                                                                            options:NSDirectoryEnumerationSkipsHiddenFiles
                        ...
                        for (NSURL* unterorderurl1 in OrdnerArray1 )
                        {
                           ...
                           if([Filemanager fileExistsAtPath:[unterorderurl1 path] isDirectory:&isDir] )
                           {
                              ...
                              if (isDir)
                              {
                                 ...
                                 {
                                    //[sammlungArray addObject:[[titel path] lastPathComponent]];
                                    [Film_Ordner addObject:[titel path]];
                                 }
                              }
                              else
                              {
                                 [Film_Ordner addObject:[unterorderurl1 path]];
                              }
                  ...
                  {
                     [Film_Ordner addObject:[unterorderurl0 path]];
                  }
               }
      ...
      else
      {
         ...
         NSLog(@"FilmarrayVonURL %@ nicht da",Genre_URL);
      }
      //NSLog(@"Film_Ordner: %@",Film_Ordner );
...
- (NSArray*)FilmarrayVonURL:(NSURL*)HD_URL
{
   NSLog(@"FilmarrayVonURL URL: %@",HD_URL);
   ...
   if ([Filemanager fileExistsAtPath:[HD_URL path]])//ist
   {
      NSString* filmVolume = [[HD_URL path] lastPathComponent];
      NSLog(@"HD_URL: %@ filmVolume: %@",HD_URL,filmVolume);
      [Volumes_Array addObject:[NSDictionary dictionaryWithObjectsAndKeys:[HD_URL path] , @"path",filmVolume,@"volume", nil]];
      ...
      NSArray* HD_OrdnerArray=  [Filemanager contentsOfDirectoryAtURL:HD_URL
                                                includingPropertiesForKeys:[NSArray arrayWithObject:NSURLNameKey]
                                                                   options:NSDirectoryEnumerationSkipsHiddenFiles
                                                                     error:&err];
      //NSLog(@"HD_OrdnerArray: %@",HD_OrdnerArray);
   ...
   else
   {
      ...
      NSLog(@"FilmarrayVonURL %@ nicht da",HD_URL);
   }
   //NSLog(@"Film_Ordner: %@",Film_Ordner );
...
{
   NSLog(@"TV_HD_A Pfad: %@ URL: %@",self.TV_HD_A_Pfad,[NSURL fileURLWithPath:self.TV_HD_A_Pfad]);
   return [self FilmarrayVonURL:[NSURL fileURLWithPath:self.TV_HD_A_Pfad]];
...
- (IBAction)reportSuchen:(id)sender;
{
   ...
   // gespeicherte Filmliste
   
   NSLog(@"suchen nach Titel: %@* ",suchstring);
   for (NSString* listepfad in ListeTitelArray)
   {
       //NSLog(@"listepfad: %@", listepfad);
      if ([listepfad rangeOfString:suchstring options:NSCaseInsensitiveSearch].location < NSNotFound)
     // if ([listepfad containsString:suchstring])
      {
          BOOL film_mounted = [Filemanager fileExistsAtPath:listepfad];
         NSLog(@"Film %@* da: %@ film_mounted: %d",suchstring, listepfad, film_mounted);
        
         
         NSMutableDictionary* findDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:[listepfad lastPathComponent],@"titel",listepfad, @"path",listepfad, @"url", [NSNumber numberWithInt:0], @"mark",[NSNumber numberWithInt:film_mounted], @"playok", nil];
         [filmArray addObject:findDic];
         if ([self.resultatfeld.stringValue length])
         {
            self.resultatfeld.stringValue = [NSString stringWithFormat:@"%@ \n %@",self.resultatfeld.stringValue,listepfad];
         }
         else
         {
            self.resultatfeld.stringValue = listepfad;
   ...
   }// for listepfad in ListeTitelArray
   ...
   for (NSString* key0 in childrenKeyArray)
   {
      ...
      for (NSString* key1 in childrenKey1Array)
      {
         ...
         if ([key1 rangeOfString:suchstring options:NSCaseInsensitiveSearch].length)
         {
            ...
            NSMutableDictionary* findDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:key1,@"titel",[node1 URL], @"url",node1, @"path", [NSNumber numberWithInt:0], @"mark",[NSNumber numberWithInt:1], @"playok", nil];
//            [filmArray addObject:findDic];
            if ([self.resultatfeld.stringValue length])
            {
...
- (IBAction)reportDeleteVonTable:(id)sender
{
   ...
   if ([alert runModal] == NSAlertFirstButtonReturn)
   {
      ...
      else
      {
         ...
         NSLog(@"trasherfolg B: %d",trasherfolg);
      }
   }
}
...
- (void)writeTitelToKellerliste:(NSString*)kellertitel
{
   ...
   NSMutableCharacterSet *customcharacterSet = [NSMutableCharacterSet whitespaceCharacterSet];
   [customcharacterSet addCharactersInString:@","];
   tempkellertitel = [tempkellertitel stringByTrimmingCharactersInSet:customcharacterSet];
...
- (NSString*)Filmtitelsauber:(NSString*)titel
{
   ...
   while ([divregexstring length]&&[divregexstring characterAtIndex:0]==' ')
   {
      divregexstring = [divregexstring substringFromIndex:1];
   }
   if ([divregexstring length]&&[divregexstring characterAtIndex:0]==',')
   {
      divregexstring = [divregexstring substringFromIndex:1];
   }
   while ([divregexstring length]&&[divregexstring characterAtIndex:0]==' ')
...
- (IBAction)reportDouble:(id)sender
{
  ...
  // NSLog(@"reportDouble Filme auf TV_HD_A");
   ...
   for (NSString* archivfilm in ListeTitelArray) // Inhalt der Files auf den externen HD
   {
 ...
 //     NSLog(@"reportDouble titelDicArray titel: %@ path: %@",[[titelDicArray  objectAtIndex:i] valueForKey:@"titel"],[[titelDicArray  objectAtIndex:i] valueForKey:@"path"]);
   ...
   int suchfilmindex=0;
   NSString* lastzeilenpfad =[NSString string];
            ...
            if (([zeilentitel rangeOfString:suchfilm].length) && ([suchfilm length]==[zeilentitel length]))
            {
               ...
               else
               {
                  ...
                  else // weiteres Vorkommen des Filmtitels
                  {
                     ...
                     NSMutableDictionary* secondDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:suchfilm,@"titel",zeilenpfad, @"path",zeilenpfad, @"url",[NSNumber numberWithInt:0], @"mark",[NSNumber numberWithInt:1], @"playok", nil];
                     ...
                     // Doppel eintragen
                     NSMutableDictionary* doppelDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:zeilentitel,@"titel",zeilenpfad, @"path", lastzeilenpfad, @"lastpath", nil];
                     [filmArray addObject:doppelDic];
                     lastzeilenpfad =[NSString string];
                     // Filmtitel in Doppelordner eintragen, damit er nicht noch einmal als erstes Vorkommen gecheckt wird
                     [doppelOrdner addObject:doppelDic];
 ...
 //  NSString* doppelstring = [[doppelOrdner valueForKey:@"path"]componentsJoinedByString:@"\n"];
   NSString* doppelstring = [doppelOrdner componentsJoinedByString:@"\n"];
   NSString* doppellistePfad=[NSHomeDirectory() stringByAppendingFormat:@"%@%@%@",@"/Documents",@"/WDTVDaten",@"/Film_DoppelListe.txt"];
   NSError* err;
   if ([[NSFileManager defaultManager]fileExistsAtPath:doppellistePfad])
   {
      [[NSFileManager defaultManager]removeItemAtPath:doppellistePfad error:nil];
   }
   BOOL suc = [doppelstring writeToFile: doppellistePfad atomically:YES encoding:NSUTF8StringEncoding error:&err];
   NSLog(@"write newTitelString suc: %d error: *%@*",suc,err);
...
- (void)volumepopwahl
{
   ...
   NSString* popvolume = [VolumePop itemTitleAtIndex:0];
   
   NSUInteger volumeindex = [[Volumes_Array valueForKey:@"volume"]indexOfObject:popvolume];
...
- (IBAction)reportVolumePop:(id)sender
{
   ...
   NSString* popvolume = [sender titleOfSelectedItem];
   NSUInteger volumeindex = [[Volumes_Array valueForKey:@"volume"]indexOfObject:popvolume];
   NSLog(@"reportVolumePop selectedItem: %ld popvolume: %@ volumeindex: %ld",(long)[sender indexOfSelectedItem],popvolume,volumeindex);
   if (volumeindex < NSNotFound)
   {
      NSLog(@"reportVolumePop  volume: %@",[Volumes_Array objectAtIndex:volumeindex]);
      NSString* volumepfad = [[[Volumes_Array objectAtIndex:volumeindex]objectForKey:@"path"]stringByDeletingLastPathComponent];
      ...
      rootNode = [[FileSystemNode alloc] initWithURL:[NSURL fileURLWithPath:self.rootNodePath]];
     // NSLog(@"rootNode: %@",[rootNode description]);
...
- (int)moveToTrash:(NSString*) pfad
{
   NSURL *url = [NSURL fileURLWithPath:pfad];
            ...
            row:(long)row
{
   ...
   if ([[tableColumn identifier] isEqual: @"delete"])
   {
      [[tableColumn dataCellForRow:row]setEnabled:playok];
   }
   if ([[tableColumn identifier] isEqual: @"mag"])
   {
      [[tableColumn dataCellForRow:row]setEnabled:playok];
   }
